...
const executeOperation = (input: string, operation: Operation): string => {
	if(operation.from.startsWith(" ")) {
		const content = operation.from.length == 1 ? "" : operation.from.substring(1, operation.from.length - 1);
...
export const executeOperations = (input: string, operations: Operation[]): string => {
	let isLooping = true;
